...
public class ItChat extends AppCompatActivity {
    private FirebaseListAdapter<ChatMessages> adapter;

    @Override
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.it_chat);

        FloatingActionButton fab =
                (FloatingActionButton)findViewById(R.id.send8);

        fab.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                EditText input = (EditText)findViewById(R.id.input8);
    ...
    private void displayChatMessages(){
        ListView listOfMessages = (ListView)findViewById(R.id.show8);

        adapter = new FirebaseListAdapter <ChatMessages>(this, ChatMessages.class,
                R.layout.messages, FirebaseDatabase.getInstance().getReference("IT")) {
            @Override
