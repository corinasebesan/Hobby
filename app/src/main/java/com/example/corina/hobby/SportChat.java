...
public class SportChat extends AppCompatActivity {
    private FirebaseListAdapter<ChatMessages> adapter;

    @Override
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.sport_chat);

        FloatingActionButton fab =
                (FloatingActionButton)findViewById(R.id.send2);
    ...
    private void displayChatMessages(){
        ListView listOfMessages = (ListView)findViewById(R.id.show2);

        adapter = new FirebaseListAdapter <ChatMessages>(this, ChatMessages.class,
                R.layout.messages, FirebaseDatabase.getInstance().getReference("Sport")) {
            @Override
